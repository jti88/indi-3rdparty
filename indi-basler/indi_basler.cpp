...
static BaslerCCD *cameras[MAX_DEVICES];
...
void ISInit()
{
       ...
       for (struct usb_device *dev = bus->devices; dev && cameraCount < MAX_DEVICES; dev = dev->next) {
         ...
         for (int i = 0; deviceTypes[i].pid; i++) {
           if (vid == deviceTypes[i].vid && pid == deviceTypes[i].pid) {
             cameras[i] = new BaslerCCD(dev, deviceTypes[i].name);
             break;
           }
         }
       }
     ...
     */
        ...
        cameras[0]             = new BaslerCCD(dev, deviceTypes[0].name);
        cameras[1]             = new BaslerCCD(dev, deviceTypes[1].name);
...
BaslerCCD::BaslerCCD(DEVICE device, const char *name)
{
    this->device = device;
...
const char *BaslerCCD::getDefaultName()
{
    return "Generic CCD";
}

bool BaslerCCD::initProperties()
{
...
void BaslerCCD::ISGetProperties(const char *dev)
{
    INDI::CCD::ISGetProperties(dev);
}

bool BaslerCCD::updateProperties()
{
...
int BaslerCCD::SetTemperature(double temperature)
{
...
bool BaslerCCD::StartExposure(float duration)
{
...
bool BaslerCCD::UpdateCCDFrameType(INDI::CCDChip::CCD_FRAME fType)
{
...
bool BaslerCCD::UpdateCCDFrame(int x, int y, int w, int h)
{
...
bool BaslerCCD::UpdateCCDBin(int binx, int biny)
{
    /**********************************************************
   *
   *
...
IPState BaslerCCD::GuideNorth(uint32_t ms)
{
    INDI_UNUSED(ms);
    /**********************************************************
   *
...
IPState BaslerCCD::GuideSouth(uint32_t ms)
{
    INDI_UNUSED(ms);
    /**********************************************************
     *
...
IPState BaslerCCD::GuideEast(uint32_t ms)
{
    INDI_UNUSED(ms);
    /**********************************************************
     *
...
IPState BaslerCCD::GuideWest(uint32_t ms)
{
    INDI_UNUSED(ms);
    /**********************************************************
     *
